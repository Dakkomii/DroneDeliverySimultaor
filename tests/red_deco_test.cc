...
class RedDecoratorTest : public ::testing::Test {
    protected:
  virtual void SetUp() {
    ...
    JsonHelper::AddFloatToJsonObject(obj2, "id", 2);
...
TEST_F(RedDecoratorTest, GreenDecoratorTest){
    drone->decorate();
    EntityBase* redDrone = new RedEntity(drone);
    redDrone->decorate();

    obj = redDrone->GetDetails();
    EXPECT_EQ(JsonHelper::GetString(obj, "color"), "0xFF0000");


    robot->decorate();
    EntityBase* redRobot = new RedEntity(robot);
    redRobot->decorate();

    obj2 = redRobot->GetDetails();
    EXPECT_EQ(JsonHelper::GetString(obj2, "color"), "0xFF0000");
